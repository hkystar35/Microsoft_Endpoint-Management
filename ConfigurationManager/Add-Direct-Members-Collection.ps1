...
function AddMembersToCollection
{
  ...
  foreach ($newMember in $newMembers.GetEnumerator())  
  {
    ...
    else
    {
      ...
      { 
        $resource = gwmi -ComputerName $SccmServer -Namespace $SccmNamespace -Class "SMS_R_User" -Filter "UniqueUserName = '$($newMember.Value.ToString())'" | select name,resourceid
      }
      if ($resource -ne $null)
      {
        if ($collectionType -eq 'DEVICE') 
...
function UIMain
{
  ...
  $collectionMembersLabel.Text = "Paste Collection Members Below **Use double backslash** like AS\\bob.smith or SWUK\jane.doe"
