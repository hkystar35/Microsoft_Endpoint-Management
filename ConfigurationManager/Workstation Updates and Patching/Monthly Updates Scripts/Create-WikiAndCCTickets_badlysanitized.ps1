...
(
    ...
    [Parameter(Mandatory = $false)][ValidateNotNullOrEmpty()][string]$CyberArkUserName = '',
    ...
    [Parameter(Mandatory = $false)][ValidateNotNullOrEmpty()][System.IO.DirectoryInfo]$<#sanitized#>CyberArkPath = "C:\Modules\<#sanitized#>.cyberark",
    [Parameter(Mandatory = $false)][ValidateNotNullOrEmpty()][string]$Confluence_UserName = '',
...
BEGIN {
	...
	$<#sanitized#>CyberArkPath_Module = Join-Path $<#sanitized#>CyberArkPath 'Source\.CyberArk.psm1'
	$<#sanitized#>CyberArkPath_CredMan = Join-Path $<#sanitized#>CyberArkPath 'Source\CredMan.ps1'
	Test-Path $<#sanitized#>CyberArkPath_Module -PathType Leaf -ErrorAction Stop
	Test-Path $<#sanitized#>CyberArkPath_CredMan -PathType Leaf -ErrorAction Stop
	...
	$GetModule = Get-Module -Name <#sanitized#>.CyberArk -ErrorAction SilentlyContinue
	IF (!$GetModule) {
		Write-Log -Message "<#sanitized#>.CyberArk module not imported."
		#$<#sanitized#>CyberArk = Find-Module -Name <#sanitized#>.CyberArk -ErrorAction Stop
		#$<#sanitized#>CyberArk | Import-Module -Force -ErrorAction Stop
		#Install-Module -Name <#sanitized#>.cyberark -Repository
		Import-Module $<#sanitized#>CyberArkPath_Module -Force -Cmdlet:$false -ErrorAction Stop
		TRY {
			$GetModule = Get-Module -Name <#sanitized#>.CyberArk -ErrorAction Stop
			Write-Log -Message "<#sanitized#>.CyberArk module found: $($GetModule.Name)."
		} CATCH {
...
PROCESS {
    ...
    TRY {
		...
		$RegularCreds = & $<#sanitized#>CyberArkPath_CredMan -GetCred -Target $env:COMPUTERNAME -User "<#sanitized#>\$($CyberArkUserName)"
        $RegularCreds_Confluence = & $<#sanitized#>CyberArkPath_CredMan -GetCred -Target $env:COMPUTERNAME -User "$($CyberArkUserName)"
        ...
        <#sanitized#>.CyberArk\Connect-CyberArk -Credential $MyCredential -Url 'https://cyberark.<#sanitized#>.com'
		$Slack = <#sanitized#>.CyberArk\Get-Account -SafeName $CyberArkSafeName -Name $CyberArkName_Slack
        $SlackToken = $Slack.Password
		$Zendesk = <#sanitized#>.CyberArk\Get-Account -SafeName $CyberArkSafeName -Name $CyberArkName_ZD
        $ZDToken = $Zendesk.Password
        $ZDURL = $Zendesk.Properties.accounts.Properties | ?{$_.key -like "*address*"} | select -ExpandProperty value
        [string]$ZDSubmitEmail = $Confluence_UserName + '@<#sanitized#>.com'
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        $ConfluenceConnect = Set-ConfluenceInfo -BaseURI 'https://wiki.<#sanitized#>.com' -Credential $MyCredential_Confluence
	...
	$RegularCreds = & $<#sanitized#>CyberArkPath_CredMan -GetCred -Target $env:COMPUTERNAME -User "<#sanitized#>\$($Confluence_UserName)"
    IF(!$RegularCreds){
...
https://wiki.<#sanitized#>.com/display/EUCENG/Out-of-Band+Patch+Schedule
...
https://wiki.<#sanitized#>.com/display/EUCENG/Windows+Updates
...
"
		...
		$Token = $ZDToken
		$URL = 'https://<#sanitized#>.zendesk.com/api/v2'
		...
		FOR ($i = 2; $i -le 9; $i++) {
			...
			SWITCH ($EmergencySchedule) {
				$true { <# Expedited schedule https://wiki.<#sanitized#>.com/display/EUCENG/Out-of-Band+Patch+Schedule #>
					SWITCH ($i) {
						2 {
				...
				$false { <# Regular schedule https://wiki.<#sanitized#>.com/display/EUCENG/Windows+Updates#WindowsUpdates-ScheduleTables #>
					SWITCH ($i) {
						2 {
							$deployDate = Get-Date -Date $Patchtuesday.AddDays(6) -UFormat %Y-%m-%d
						}
			...
			$body = [PSCustomObject]@{
				ticket = [PSCustomObject]@{
					...
					fields  = [PSCustomObject]@{
						114094622752 = '1'
						31247267 = 'https://wiki.<#sanitized#>.com/display/EUCENG/Windows+Updates#WindowsUpdates-RollbackPlan'
						33025407 = "$($conflvar.url)"
						30330248 = 'cc_risk_medium'
						31213927 = 'N/A'
						42880248 = 'https://wiki.<#sanitized#>.com/display/EUCENG/Windows+Updates#WindowsUpdates-ValidationandMonitoring'
						33025387 = 'N/A'
						32282328 = 'N/A'
