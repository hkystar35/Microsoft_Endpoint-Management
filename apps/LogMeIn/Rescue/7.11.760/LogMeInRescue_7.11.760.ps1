...
Try {
	...
	[string]$appVendor = 'LogMeIn, Inc.'
	[string]$appName = 'LogMeIn Rescue Calling Card'
	[string]$appVersion = '7.11.760'
	[string]$appMSIProductCode = '{84EEC5C4-7E98-0EA0-05DC-35654E64F0DE}'
	[string]$appProcessesString = 'CallingCard' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-MSI -Action Install -Path 'LogMeIn Rescue Calling Card.msi' -Transform 'LMI_Rescue.mst'
		...
		$CustomName = 'Ask EUC - LogMeIn'
		Remove-DesktopShortcut -N $CustomName -WC
		
		#region Create FireWall Rules - Inbound
		$FirewallDisplayName = "LogMeIn Rescue"
		
		$FirewallRuleDescription = $CustomName
		
		$ExeName = 'callingcard.exe'
		$TargetFile = Join-Path -Path (Get-InstalledApplication -Name "$CustomName" | Select-Object -ExpandProperty InstallLocation) -ChildPath $ExeName
		$Protocols = "TCP", "UDP"
		
		#Remove Existing Firewall Rules
		$GetExistingRules = Get-NetFirewallRule | Where-Object{$_.DisplayName -like "*$FirewallDisplayName*"}
		IF ($GetExistingRules) {
		...
		IF (Test-Path -Path $TargetFile -PathType Leaf) {
			FOREACH ($Protocol IN $Protocols) {
				$NewFirewallRule = New-NetFirewallRule -DisplayName "$FirewallDisplayName" -Description "$FirewallRuleDescription" -Direction Inbound -Program "$TargetFile" -Protocol $Protocol -Action Allow -EdgeTraversalPolicy DeferToUser -Enabled True -Profile Any
				IF ($NewFirewallRule) {
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$GetExistingRules = Get-NetFirewallRule | Where-Object{
			$_.DisplayName -like "*$FirewallDisplayName*"
		}
		IF ($GetExistingRules) {
			$GetExistingRules | ForEach-Object{
				Write-Log -Message "Found Existing Firewall Rule, Deleting: $($_.DisplayName) ($($_.Name))" -Severity 1 -Source $installPhase
			...
			Write-Log -Message "No Existing Firewall Rules matching $FirewallDisplayName exist." -Severity 1 -Source $installPhase
		}
		#endregion Create FireWall Rules - Inbound
