...
Try {
	...
	[string]$appVendor = 'Iceni'
	[string]$appName = 'InFix PDF Editor'
	[string]$appVersion = '7.1.8'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '07/21/2017'
	...
	#>	
        ...
        #>
            ...
            Remove-MSIApplications -Name "Infix"
            Remove-MSIApplications -Name "Infix PDF Editor"
            Remove-MSIApplications -Name "Infix PDF Editor version"
		        $UninstallPaths = "Iceni\Infix5\unins000.exe",
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'infix' -AllowDefer -DeferTimes 3 -CheckDiskSpace -PersistPrompt
		...
		$Installversion = [System.Diagnostics.FileVersionInfo]::GetVersionInfo("$dirFiles\InfixSetup.exe").FileVersion
        Execute-Process -Path "InfixSetup.exe" -Parameters "/LOADINF=`"$dirFiles\setup.inf`" /VERYSILENT /SUPPRESSMSGBOXES /SP- /NORESTART /LOG=`"$configToolkitLogDir\$appName-$Installversion.log`""
        ...
        Set-RegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\Iceni Technology Limited\Infix' -Name 'CheckForUpdates' -Value 'false'
        Set-RegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Iceni Technology Limited\Infix' -Name 'CheckForUpdates' -Value 'false'

        ## Add license key
        # Prod Code - 2563343301
        # Prod Key - 869c1c84-88b6-4503-b00b-c32d0ceebc87
		$LicFolderPath = "$envProgramData\Iceni\Infix"
        $LicFolderPathVersions = "6","7"
        $LicFileName = "infix.key"
        $LicFileContent = "869c1c84-88b6-4503-b00b-c32d0ceebc87"
        foreach($LicFolderPathVersion in $LicFolderPathVersions){
            if(Test-Path "$LicFolderPath\$LicFolderPathVersion"){
                New-Item "$LicFolderPath\$LicFolderPathVersion\$LicFileName" -type file -force -value "$LicFileContent"
            }
        }
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'infix' -CloseAppsCountdown 300
        ...
        Remove-RegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\Iceni Technology Limited\Infix' -Name 'CheckForUpdates'
        Remove-RegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Iceni Technology Limited\Infix' -Name 'CheckForUpdates'
        ...
        foreach($LicFolderPathVersion in $LicFolderPathVersions){
            if(Test-Path "$LicFolderPath\$LicFolderPathVersion"){
                Remove-File -Path "$LicFolderPath\$LicFolderPathVersion\$LicFileName"
            }
        }
