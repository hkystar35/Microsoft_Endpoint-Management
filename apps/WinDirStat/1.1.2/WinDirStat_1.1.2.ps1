...
Try {
	...
	[string]$appVendor = 'WinDirStat'
	[string]$appName = 'WinDirStat'
	[string]$appVersion = '1.1.2'
	[string]$appMSIProductCode = ''
	[string]$appProcessesString = 'windirstat' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	...
	[string]$appScriptDate = '02/08/2018'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-Process -Path 'windirstat1_1_2_setup.exe' -Parameters '/S'
		...
		$Path = "$envProgramData\Microsoft\Windows\Start Menu\Programs"
		$FolderName = 'WinDirStat'
		$TargetPath = "$envProgramFilesX86\WinDirStat"
		$ExeName = 'windirstat.exe'
		if (!(Test-Path -Path $Path\$FolderName\WinDirStat.lnk -PathType Leaf)) {
			New-Shortcut -Path "$Path\$FolderName\WinDirStat.lnk" -TargetPath "$TargetPath\$ExeName" -IconLocation "$TargetPath\$ExeName" -Description 'WinDirStat'
		}
		Refresh-Desktop
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$Paths = "$envProgramFiles\WinDirStat", "$envProgramFilesX86\WinDirStat"
		$FileName = 'Uninstall.exe'
		$Paths | ForEach-Object{
			if (Test-Path -Path $_\$FileName -PathType Leaf) {
				Execute-Process -Path "$_\$FileName" -Parameters '/S'
		...
		$Path = 'AppData\Roaming\Microsoft\Windows\Start Menu\Programs\WinDirStat'
		$UserProfiles | ForEach-Object{
			if (Test-Path -Path "$_\$Path" -Type Container) {
				$Items = Get-ChildItem -Path "$_\$Path" | Measure-Object
				if ($Items.count -eq 0) {
					Remove-Folder -Path "$_\$Path"
				}
			}
		}
