...
Try {
	...
	[string]$appVendor = 'Node.js Foundation'
	[string]$appName = 'Node.js'
	[string]$appVersion = '8.9.1'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '11/20/2017'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'node' -AllowDefer -DeferTimes 3 -CheckDiskSpace -PersistPrompt
        ...
        Switch($Is64Bit){
            True{$InstallFileName = 'node-v8.9.1-x64.msi'
            $Bitness = '64-bit'
            }
            False{$InstallFileName = 'node-v8.9.1-x86.msi'
            $Bitness = '32-bit'
            }
        }
        
        Execute-MSI -Action Install -Path "$InstallFileName"
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'node' -CloseAppsCountdown 300
        ...
        #x86
        Execute-MSI -Action Uninstall -Path {883ECC46-3EED-4960-B912-1CFAF4A8BDB7}
        #x64 
        Execute-MSI -Action Uninstall -Path {3DF75316-DB80-4E5E-9D48-0288AE6050D1}
