...
Try {
	...
	[string]$appName = 'Java 8 Update 201'
	[string]$appVersion = '8.0.2010.9'
	...
	[string]$appScriptDate = '02/19/2019'
...
"@
		#REMOVEOUTOFDATEJRES=1 uninstalls all old JREs, 0 leaves old
		$CFGfile = New-Item -Path "$configToolkitLogDir" -Name "$($appName)_$($appVersion).cfg" -ItemType File -Value $JREcfg -Force
		Execute-Process -Path 'jre-8u201-windows-i586.exe' -Parameters "INSTALLCFG=`"$($CFGfile.FullName)`" /s /L `"$configToolkitLogDir\$($appName)_$($appVersion).log`"" -WaitForMsiExec
		Execute-Process -Path 'jre-8u201-windows-x64.exe' -Parameters "INSTALLCFG=`"$($CFGfile.FullName)`" /s /L `"$configToolkitLogDir\$($appName)_$($appVersion).log`"" -WaitForMsiExec
		
		$JavaUpdaters = Get-InstalledApplication -Name 'Java Auto Updater'
		foreach($JavaUpdater in $JavaUpdaters){
			Execute-MSI -Action Uninstall -Path $($JavaUpdater.ProductCode)
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$JavaUninstallers = Get-InstalledApplication -Name 'Java'
		foreach($JavaUninstaller in $JavaUninstallers){
			IF($JavaUninstaller.DisplayVersion -eq $appVersion){
				Execute-MSI -Action Uninstall -Path $($JavaUninstaller.ProductCode)
			}
		}
