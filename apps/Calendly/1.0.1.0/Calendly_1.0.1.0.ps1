...
Try {
	...
	[string]$appVendor = 'Calendly'
	[string]$appName = 'Calendly for Outlook'
	[string]$appVersion = '1.0.1.0'
	[string]$appMSIProductCode = <#$false #> '{B63CC63C-F21F-4EAE-AD72-EF0931FFA83B}'
	[string]$appProcessesString = <#$false #>'calendly,calendlypush' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = $false #'' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '02/11/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		# Remove previous Calendly installs
		Remove-MSIApplications -Name 'Calendly'
		Remove-MSIApplications -Name 'Calendly for Outlook'
		Execute-MSI -Action Uninstall -Path '{993E010F-D45F-4791-8A4F-D9E533B59EF3}' # Previous version. Uses same version number but different GUID
		
		Execute-MSI -Action Install -Path 'Calendly_Outlook_plugin_149.msi'
		#Execute-Process -Path 'Calendly_Outlook_plugin_149.exe' -Parameters "/quiet /norestart /log `"$configToolkitLogDir\$($appName)_$($appVersion)_Install.log`""
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Get-MSIinfo -Path E:\git\apps\Calendly\1.0.1.0_new\Files\Calendly_Outlook_plugin_149.msi -Property ProductCode
		##*===============================================
		##* POST-UNINSTALLATION
		##*===============================================
