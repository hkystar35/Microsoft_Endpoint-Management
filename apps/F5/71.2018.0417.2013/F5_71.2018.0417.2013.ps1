...
Try {
    ...
    [string]$appVendor = 'F5 Networks, Inc.'
    [string]$appName = 'BIG-IP Edge Client'
    [string]$appVersion = '71.2018.0417.2013'
    [string]$appMSIProductCode = $false #''
    [string]$appProcessesString = $false #'f5fpclientW' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
    ...
    [string]$appScriptDate = '06/24/2019'
    ...
    If ($deploymentType -ine 'Uninstall') {
        ...
        if ($appProcessesString) {
            ...
            Show-InstallationWelcome -DeferTimes 3 -CheckDiskSpace -PersistPrompt
        }
        ...
        Execute-MSI -Action Install -Path 'BIGIPComponentInstaller.msi'
        Execute-Process -Path 'BIGIPWinEdgeClient.exe' -Parameters '/q' -WaitForMsiExec
        ...
        ## Delete Startup Keys
        $RegName = 'F5_SAM_Client'
        Remove-RegistryKey -Key HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\StartupApproved\Run32 -Name $RegName
        Remove-RegistryKey -Key HKLM\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Run -Name $RegName
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        Show-InstallationWelcome -CloseApps 'f5fpclientW=F5 VPN Client' -CloseAppsCountdown 300
        ...
        $Services = 'F5 Networks Component Installer',
        'F5CredMgrSrv',
        'F5FltSrv',
        'F5InspectorService',
        'F5TrafficSrv'
        foreach ($Service in $Services) {
            Stop-ServiceAndDependencies -Name $Service 
        }
        
        $Processes = 'F5CredMgrSrv',
        'F5FltSrv',
        'f5InspectorService',
        'F5InstallerService',
        'F5MachineCertService',
        'F5TrafficSrv'
        foreach ($Process in $Processes) {
            $Message = Stop-Process -Name $Process -ErrorAction SilentlyContinue -Force -PassThru
            Write-Log -Message "Stopped process $($Message.ProcessName)"
            Clear-Variable Message -Force
        }
			
        Remove-MSIApplications -Name 'BIG-IP Edge Client'
        Remove-MSIApplications -Name 'BIG-IP Component Installer'
