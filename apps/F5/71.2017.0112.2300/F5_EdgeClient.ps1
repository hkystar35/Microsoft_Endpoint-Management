...
Try {
	...
	[string]$appVersion = '71.2017.0112.2300'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'f5fpclientW' -AllowDefer -DeferTimes 3 -CheckDiskSpace -PersistPrompt
		...
		Execute-Process -Path 'BIGIPWinEdgeClient.exe' -Parameters '/q'
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
        ...
        $Uninstaller = "$envProgramData\F5 Networks\f5unistall.exe"
        if(Test-Path "$Uninstaller"){
            Execute-Process -Path "$Uninstaller" -Parameters '/uninstall /r'
		}
        Execute-MSI -Action Uninstall -Path '{6D4839CB-28B4-4070-8CA7-612CA92CA3D0}'
