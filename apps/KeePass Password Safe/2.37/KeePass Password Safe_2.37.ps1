...
Try {
	...
	[string]$appName = 'KeePass 2.37'
	[string]$appVersion = '2.37.0'
	[string]$appMSIProductCode = '{82D10083-5573-4155-A9C2-7324539E63BD}'
	[string]$appProcessesString = 'keepass' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = '' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '12/26/2017'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		# Remove non-MSI installers first
		$AppFolders = 'KeePass Password Safe 2', 'KeePass Password Safe', 'KeePass2x', 'KeepassXC'
		$AppUninstallers = 'Uninstall.exe', 'unins000.exe'
		$AppFolders | ForEach-Object{
			foreach ($AppUninstaller in $AppUninstallers) {
				if (Test-Path -LiteralPath "$envProgramFilesX86\$_\$AppUninstaller" -PathType Leaf) {
					Execute-Process -Path "$envProgramFilesX86\$_\$AppUninstaller" -Parameters '/VERYSILENT'
				}
				if (Test-Path -LiteralPath "$envProgramFiles\$_\$AppUninstaller" -PathType Leaf) {
					Execute-Process -Path "$envProgramFiles\$_\$AppUninstaller" -Parameters '/VERYSILENT'
				}
			}
		}
		...
		Execute-MSI -Action Install -Path 'KeePass-2.37.msi'
		...
		$ConfigFile = 'KeePass.config.xml'
		$AppFolders = 'KeePass Password Safe 2', 'KeePass Password Safe', 'KeePass2x'
		$AppFolders | ForEach-Object{
			if (Test-Path -LiteralPath "$envProgramFilesX86\$_") {
				Copy-File -Path "$dirFiles\$ConfigFile" -Destination "$envProgramFilesX86\$_\$ConfigFile"
			}
			if (Test-Path -LiteralPath "$envProgramFiles\$_") {
				Copy-File -Path "$dirFiles\$ConfigFile" -Destination "$envProgramFilesX86\$_\$ConfigFile"
			}
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Execute-MSI -Action Install -Path "$appMSIProductCode"
