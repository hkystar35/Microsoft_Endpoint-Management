...
Try {
	...
	[string]$appVendor = 'Minitab, LLC'
	[string]$appName = 'Minitab 19'
	[string]$appVersion = '19.1.1.0'
	[string]$appMSIProductCode = '{602882B8-9583-4466-9724-B5A79661BA98}'
	[string]$appProcessesString = 'mtb,borrow' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = $false #'' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '09/09/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$Shortcut = Get-DesktopShortcut -Name "Minitab*"
		...
		Execute-Process -Path 'minitab19.1.1.0setup.x64.exe' -Parameters "/exenoui /qn ACCEPT_EULA=1 LICENSE_SERVER=Int-Minitab-01 DISABLE_ANALYTICS=1 DISABLE_UPDATES=1 /L*V `"$configToolkitLogDir\$($appName)_$($appversion)_$($installPhase).log`""
		...
		IF(!$Shortcut){
			Remove-DesktopShortcut -Name "Minitab*"
		}
