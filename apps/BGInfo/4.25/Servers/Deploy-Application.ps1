...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		# Config file name
		$BGInfoConfigFile = 'PCTY_Servers.bgi'
		...
		# Set batch file content
		$BGInfoBatchContent = @"
:: Created on $currentDateTime
@ECHO OFF
"$BGInfoDestination\$BGInfoFile" "$BGInfoDestination\$BGInfoConfigFile" /TIMER:0 /SILENT /NOLICPROMPT
"@
		# Create batch file in Startup folder
		New-Item -Path "$envCommonStartUp" -ItemType File -Name $BGInfoBatch -Value $BGInfoBatchContent -Force
		
		# Execute batch for first time
		IF (Test-Path -Path "$envCommonStartUp\$BGInfoBatch" -PathType Leaf) {
			Execute-Process -Path "$envCommonStartUp\$BGInfoBatch" -NoWait
		} ELSEIF (!(Test-Path -Path "$envCommonStartUp\$BGInfoBatch" -PathType Leaf)) {
			Write-Log -Message "Failed to find "$envCommonStartUp\$BGInfoBatch"!" -Severity 3
