...
Try {
	...
	[string]$appVendor = ''
	[string]$appName = 'BGInfo'
	[string]$appVersion = '4.25'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		# Config file name
		$BGInfoConfigFile = 'PCTY_Workstations_Popup.bgi'
		
		# Set names and paths based on OS bitness
		IF ($envOSArchitecture -eq '64-bit') {
			$BGInfoFile = 'Bginfo64.exe'
			#$BGInfoBatch = 'BGInfo64_PCTY.bat'
			$BGInfoDestination = "$envProgramFiles\BGInfo"
		} ELSE {
			$BGInfoFile = 'Bginfo.exe'
			#$BGInfoBatch = 'BGInfo_PCTY.bat'
			$BGInfoDestination = "$envProgramFilesX86\BGInfo"
		}
		
		# Copy files to Program Files
		Copy-File -Path "$dirFiles\$BGInfoFile" -Destination "$BGInfoDestination"
		Copy-File -Path "$dirFiles\$BGInfoConfigFile" -Destination "$BGInfoDestination"
		Copy-File -Path "$PSScriptRoot\AppDeployToolkit\AppDeployToolkitLogo.ico" -Destination "$BGInfoDestination"
		

		# Create batch file in Startup folder
		#New-Item -Path "$BGInfoDestination" -ItemType File -Name $BGInfoBatch -Value $BGInfoBatchContent -Force
		
		# Create shortcut
		New-Shortcut -Path "$envCommonStartMenuPrograms\PC Info.lnk" -TargetPath $BGInfoDestination\$BGInfoFile -Arguments "`"$BGInfoDestination\$BGInfoConfigFile`" /TIMER:0 /POPUP /SILENT /NOLICPROMPT" -IconLocation "$BGInfoDestination\AppDeployToolkitLogo.ico" -Description 'Displays computer information' -WindowStyle Normal
