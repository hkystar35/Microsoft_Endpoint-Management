...
Try {
	...
	[string]$appVendor = 'Intevation GmbH'
	[string]$appName = 'GPG4Win'
	[string]$appVersion = '3.1.2.2814'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-Process -Path 'gpg4win-3.1.2.exe' -Parameters "/S /C=$($dirFiles)\gpg4win.ini"
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$arguments = "/S"# /O `"$configToolkitLogDir\$($appName)_$($appVersion)_Uninstall.log`""
		...
		} ELSEIF (Test-Path -Path "$envProgramFilesX86\Gpg4win\gpg4win-uninstall.exe" -PathType Leaf) {
			Write-Log -Message "Used if-then x86" -Severity 1 -Source $installPhase
			Execute-Process -Path "$envProgramFilesX86\Gpg4win\gpg4win-uninstall.exe" -Parameters $arguments
		} ELSEIF (Test-Path -Path "$envProgramFiles\Gpg4win\gpg4win-uninstall.exe" -PathType Leaf) {
			Write-Log -Message "Used if-then" -Severity 1 -Source $installPhase
			Execute-Process -Path "$envProgramFiles\Gpg4win\gpg4win-uninstall.exe" -Parameters $arguments
		}
