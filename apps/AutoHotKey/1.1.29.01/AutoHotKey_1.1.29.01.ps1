...
Try {
	...
	[string]$appVendor = 'AutoHotKey'
	[string]$appName = 'AutoHotKey'
	[string]$appVersion = '1.1.29.01'
	...
	[string]$appScriptDate = '06/06/2018'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		#Execute-Process -Path 'AutoHotkeyU64.exe' -Parameters '/S'
		Execute-Process -Path 'AutoHotkey_1.1.29.01_setup.exe' -Parameters '/S /R'
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		IF ($GetInstalled) {
			...
			$arguments = '"' + $uninstaller[1] + '"' + ' /Uninstall'
		...
		} ELSEIF (Test-Path -Path "$envProgramFilesX86\AutoHotkey\AutoHotkey.exe" -PathType Leaf -and Test-Path -Path "$envProgramFilesX86\AutoHotkey\Install.ahk" -PathType Leaf) {
			Write-Log -Message "Used if-then x86" -Severity 1 -Source $installPhase
			Execute-Process -Path "$envProgramFilesX86\AutoHotkey\AutoHotkey.exe" -Parameters "$envProgramFilesX86\AutoHotkey\Install.ahk /Uninstall"
		} ELSEIF (Test-Path -Path "$envProgramFiles\AutoHotkey\AutoHotkey.exe" -PathType Leaf -and Test-Path -Path "$envProgramFiles\AutoHotkey\Install.ahk" -PathType Leaf) {
			Write-Log -Message "Used if-then" -Severity 1 -Source $installPhase
			Execute-Process -Path "$envProgramFiles\AutoHotkey\AutoHotkey.exe" -Parameters "$envProgramFilesX86\AutoHotkey\Install.ahk /Uninstall"
		}
		##*===============================================
		##* POST-UNINSTALLATION
		##*===============================================
