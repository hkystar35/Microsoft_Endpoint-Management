...
Try {
	...
	[string]$appVendor = 'salesforce.com'
	[string]$appName = 'Salesforce for Outlook'
	[string]$appVersion = '3.4.05.33138'
	[string]$appMSIProductCode = '{3571FD3C-32B0-467F-929F-6E9F8F0FCC68}'
	[string]$appProcessesString = 'outlook,salesforce,SfdcMsOl,calendly' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = $false #'' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '05/31/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		# Get existing shortcuts, if exist
		$ShortcutString = 'Salesforce for Outlook'
		$ExistingShortcuts = Get-DesktopShortcut -Name $ShortcutString

		$OfficeInfo = Get-OfficeInfo | ?{$_.Bitness -ne $null -or $_.Bitness -ne ''} | Sort-Object Version -Descending | Select-Object -First 1
		
		SWITCH($OfficeInfo.bitness){
			x64 {$InstallerName = "SalesforceForOutlook_x64.msi"}
			x86 {$InstallerName = "SalesforceForOutlook_x86..msi"}
		}
		Execute-MSI -Action Install -Path $InstallerName
		
		# Remove Desktop shortcuts if they weren't already there
		$NewShortcuts = Get-DesktopShortcut -Name $ShortcutString
		$NewShortcuts | Where-Object {$_ -notin $ExistingShortcuts} | ForEach-Object{
			Remove-File -Path $_
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Remove-MSIApplications -Name "$ShortcutString"
		Execute-MSI -Action Uninstall -Path $appMSIProductCode
		
		# Remove Desktop shortcuts
		$Shortcuts = Get-DesktopShortcut -Name $ShortcutString
