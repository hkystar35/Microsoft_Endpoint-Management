...
Try {
	...
	[string]$appVendor = 'Proofpoint, Inc.'
	[string]$appName = 'PhishAlarm Outlook Add-In'
	[string]$appVersion = '3.2.37.0'
	[string]$appMSIProductCode = '{6C15F7E8-2100-4C21-9119-6B2E04926312}'
	...
	[string]$appScriptDate = '10/23/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		if ($appProcessesString) {
			#Show-InstallationWelcome -CloseApps "$appProcessesString" -AllowDeferCloseApps -CheckDiskSpace -PersistPrompt -BlockExecution -ForceCloseAppsCountdown 6900 -MinimizeWindows $false -DeferTimes 3
			#Show-InstallationWelcome -CloseApps "$appProcessesString" -AllowDeferCloseApps -CheckDiskSpace -PersistPrompt -BlockExecution -ForceCloseAppsCountdown 6900 -MinimizeWindows $false -DeferDays 10
			Show-InstallationWelcome -CloseApps "$appProcessesString" -AllowDeferCloseApps -CheckDiskSpace -PersistPrompt -BlockExecution -ForceCloseAppsCountdown 6900 -MinimizeWindows $false -DeferDeadline ([datetime]"December 13, 2019 09:00:00")
		} else {
		...
		<#
			HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Wombat Security\PhishAlarm Outlook Add-In
			CompanyId = "e73efd97-fec2-458c-0000-b924-74f494715de2"
		#>
		
		Execute-Process -Path 'PhishAlarm.exe' -Parameters "/silent /norestart /log `"$configToolkitLogDir\$($appName)_$($appVersion)_$($installPhase).log`""
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Execute-Process -Path 'PhishAlarm.exe' -Parameters "/silent /uninstall /norestart /log `"$configToolkitLogDir\$($appName)_$($appVersion)_$($installPhase).log`""
