...
Try {
	...
	[string]$appName = 'Microsoft Visual Studio Tools for Office Runtime 2010 Redistributable'
	[string]$appVersion = '10.0.60724.00'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-Process -Path "vstor_redist.exe" -Parameters "/q /norestart /log `"$configToolkitLogDir\VSTOR$($appVersion)_$($installPhase).htm`""
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Execute-Process -Path "vstor_redist.exe" -Parameters "/q /uninstall /norestart /log `"$configToolkitLogDir\VSTOR$($appVersion)_$($installPhase).htm`""
