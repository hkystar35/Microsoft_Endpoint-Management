...
Try {
	...
	[string]$appName = 'OneDrive for Business'
	[string]$appVersion = '18.25.204.9'
	[string]$appMSIProductCode = ''
	[string]$appProcessesString = 'onedrive' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = '' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '05/04/2018'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		#Execute-Process -Path 'OneDriveSetup.exe' -Parameters '/Silent'
		...
		#Needs to be installed per user, not per-machine
		
		$Domain = 'Paylocity'
		$Username = 'Nwendlowsky'
		$Email = $Username + '@' + $Domain + '.com'
		#New-Shortcut -Path "odopen://sync?useremail=$Email" -
		Set-ActiveSetup -StubExePath "$envProgramFiles\internet explorer\iexplore.exe" -Arguments 'odopen://sync?useremail=%username%@paylocity.com' -Description 'OneDrive RunOnce Sign In' -Key 'OneDrive'
