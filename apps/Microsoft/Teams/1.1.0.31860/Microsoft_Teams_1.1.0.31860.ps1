...
Try {
	...
	[string]$appName = 'Teams Machine-Wide Installer'
	[string]$appVersion = '1.1.0.31860'
	[string]$appMSIProductCode = '{ED4C4DFF-8C5C-4D20-8A01-5A261D67979D}'
	[string]$appProcessesString = 'teams' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = $false #'' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '01/09/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		# $USERProfile\appdata\local\microsoft\teams\current\teams.exe
		$UserProfiles = Get-UserProfiles
		$Protocols = "TCP", "UDP"
		
		#Remove Existing Firewall Rules
		$GetExistingRules = Get-NetFirewallRule | Where-Object{ $_.DisplayName -like "*Microsoft Teams*" }
		IF ($GetExistingRules) {
		...
		FOREACH ($Profile in $UserProfiles) {
			$TargetFile = Join-Path -Path $Profile.Profilepath -ChildPath '\appdata\local\microsoft\teams\current\teams.exe'
		...
		#region Check if existing app in AppData
		# https://docs.microsoft.com/en-us/microsoftteams/scripts/powershell-script-teams-deployment-clean-up
		$ExistingPath = "$envLocalAppData\Microsoft\Teams"
		$UninstallFile = 'Update.exe'
		IF (Test-Path $ExistingPath\$UninstallFile -PathType Container -ErrorAction SilentlyContinue) {
			$Uninstall = Execute-Process -Path "$ExistingPath\$UninstallFile" -Parameters '-uninstall -s' -Passthru
			IF ($Uninstall.ExitCode -eq 0) {
				Remove-Folder -Path $ExistingPath
			} ELSE {
				Exit-Script -ExitCode 69001
			}
			
		}
		#endregion Check if existing app in AppData
		...
		Execute-MSI -Action Install -Path 'Teams_windows_x64.msi' # -AddParameters "OPTIONS=`"noAutoStart=true`""
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$Profiles = Get-UserProfiles | Where-Object{ $_.ntaccount -like "paylocity\*" }
		$Path = "AppData\Local\Microsoft\Teams"
		$UninstallFile = 'Update.exe'
		
		FOREACH ($Profile IN $Profiles) {
			IF (Test-Path "$Profile\$Path\$UninstallFile" -PathType Leaf -ErrorAction SilentlyContinue) {
				$Uninstall = Execute-Process -Path "$Profile\$Path\$UninstallFile" -Parameters '-uninstall -s' -Passthru
				IF ($Uninstall.ExitCode -eq 0) {
					Remove-Folder -Path "$Profile\$Path"
				} ELSE {
					Exit-Script -ExitCode 69001
				}
