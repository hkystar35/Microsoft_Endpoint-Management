...
Try {
	...
	[string]$appVersion = '1.2.0.26774'
	[string]$appMSIProductCode = '{731F6BAA-A986-45A4-8936-7C3AAAAA760B}'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-MSI -Action Install -Path "Teams_windows_x64.msi" -AddParameters "ALLUSERS=1" # -AddParameters "OPTIONS=`"noAutoStart=true`"" <#use if installing without autostart in Windows#>
		...
		## Do not test for file path, it will get created after user's next logon
		$FilePath = "$envLocalAppData\Microsoft\Teams\current\Teams.exe"
		Create-FirewallRule -Protocol TCPUDP -DisplayName "$appName" -Description "$appName firewall rule" -FilePath $FilePath
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$UserProfiles = Get-UserProfiles -ExcludeDefaultUser
		foreach ($UserProfile in $UserProfiles){
			$UserLOCALAPPDATA = Join-Path -Path $UserProfile.ProfilePath -ChildPath "AppData\Local"
			$TeamsPath = [System.IO.Path]::Combine($UserLOCALAPPDATA, 'Microsoft', 'Teams')
			$TeamsUpdateExePath = [System.IO.Path]::Combine($UserLOCALAPPDATA, 'Microsoft', 'Teams', 'Update.exe')
			if (Test-Path -Path $TeamsUpdateExePath) {
				Write-Log "Uninstalling Teams process from $($UserProfile.NTAccount)"
				Execute-Process -Path $TeamsUpdateExePath -Parameters "-uninstall -s"
			}
			if (Test-Path -Path $TeamsPath) {
				Write-Log "Deleting Teams directory $TeamsPath"
				Remove-Folder -Path $TeamsPath
			}
			Remove-Variable UserLOCALAPPDATA,TeamsPath,TeamsUpdateExePath -ErrorAction SilentlyContinue -Force
		}
