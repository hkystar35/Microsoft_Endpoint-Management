...
Try {
	...
	[string]$appName = 'Silverlight'
	[string]$appVersion = '5.1.50907.0'
	[string]$appMSIProductCode = '{89F4137D-6C26-4A84-BDB8-2E5A4BB71E00}'
	[string]$appProcessesString = 'iexplore' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = $false #'' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '07/20/2018'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-Process -Path 'Silverlight_x64.exe' -Parameters "/q /doNotRequireDRMPrompt /noupdate /log `"$configToolkitLogDir\$($appName)_$($appVersion)_$($installPhase).log`""
		...
		$KayPaths = 'HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Silverlight',
					'HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Microsoft\Silverlight',
					'HKEY_CURRENT_USER\SOFTWARE\Microsoft\Silverlight'
		
		foreach ($KayPath in $KayPaths) {
			Set-RegistryKey -Key $KayPath -Name 'UpdateConsentMode' -Value '0' -Type DWord
			Set-RegistryKey -Key $KayPath -Name 'UpdateMode' -Value '2' -Type DWord
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Execute-Process -Path 'Silverlight_x64.exe' -Parameters "/qu /l*v `"$configToolkitLogDir\$($appName)_$($appVersion)_$($installPhase).log`""
