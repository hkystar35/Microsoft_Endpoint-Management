...
Try {
	...
	[string]$appScriptDate = '01/03/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		## SQLnativeclient 2008
		Execute-MSI -Action Install -Path "$dirFiles\sqlnative2008\sqlncli_x64.msi" -AddParameters "IACCEPTSQLNCLILICENSETERMS=YES"
		...
		#Smart Connect
				#remove old version
				Execute-MSI -Action Uninstall -Path '{D552EE05-451E-4FF9-9ABF-FBD08D2664BB}'
				#install new if yes
					IF ($Choice -eq 'Yes') {
						Execute-Process -Path "$dirFiles\Add-ins\Smart Connect 2014 R2\SmartConnect_Installer_V2014.1.25.exe" -Parameters "/exenoui /qn ADDLOCAL=UserInterfaceF,WindowsServiceF,Gp2015F /L*v `"$configToolkitLogDir\SmartConnect_V2014.1.25_$($installPhase).log`""
					}
					#eConnect for MS Dynamics GP 2015 64bit
					existing GP install
