...
TRY {
	...
	[string]$appName = 'Microsoft Dynamics CRM 2013 for Microsoft Office Outlook'
	[string]$appVersion = '6.0.0000.0819'
	[string]$appBaseVersion = '6.0.0000.0000'
	...
	IF ($deploymentType -ine 'Uninstall') {
		...
		$CRM_URL = 'https://crm.paylocity.com'
		$CRM_Org = 'Paylocity'
		[string]$XML_Content = @"
<Deployments>
	<Deployment>
			...
			FOREACH ($App IN $OfficeInfo) {
				...
				IF ($App.Bitness) {
					...
					IF (!$DynamicsAlreadyInstalled) {
						...
						$Extract = Execute-Process -Path "$InstallerPath\CRM2013-Client-ENU-$($Bitness_hotfix).exe" -Parameters "/Extract:`"$($InstallerExtractPath)`" /quiet /log:`"$($configToolkitLogDir)\CRM_exe_ExtractFiles.log`"" -WaitForMsiExec -PassThru
						
						IF ($Extract.ExitCode -eq 0) {
							# Install base application based on bitness
							$Installer = Execute-Process -Path "$InstallerExtractPath\SetupClient.exe" -Parameters "/Q /LV `"$($configToolkitLogDir)\CRM_exe_Install.log`"" -PassThru
						} ELSE {
					...
					IF ($Hotfix) {
						...
						IF ($InstalledHotfix) {
							...
							$InstalledKB = $InstalledHotfix.UninstallSubkey.Split('_') | Where-Object{
								$_ -like "KB*"
							}
						}
						...
						Write-Log -Message "No hotfix in source files to install." -Severity 1 -Source $installPhase
					}
