...
Try {
	...
	[string]$appVendor = 'Bastien Mensink'
	[string]$appName = 'ASAP Utilities'
	[string]$appVersion = '7.4'
	[string]$appMSIProductCode = $false #''
	[string]$appProcessesString = 'excel' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = $false #'' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '06/11/2018'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-Process -Path 'ASAP_Utilities_7-4_Setup.exe' -Parameters "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /SP- /LOG=`"$configToolkitLogDir\$($appName)_$($appVersion)_Install.log`""
		...
		#region Add License Info to HKCU
		$HKCU_Names = 'name,Ian Rogers',
		'key,0175~0220~0178~0271~0154~0166~0247~0262~0184~0154~0226~0241~0232~0163~0154~0286~0184~0241~0172~',
		'order,0178~0181~0175~0181~0181~0184~0166~0187~0172~',
		'orderamount,0169~0178~0163~',
		'organization,Paylocity'
		#			$HKCU_Names = 'name,Ian Rogers',
		#			'organization,Paylocity',
		#			'order,564667183',
		#			'orderamount,250',
		#			'key,4C5T-1LQ7-EJG0-Y7J3'
		$SID = (Get-Loggedonuser).SID
		$HKCU_Names | ForEach-Object{
			$Name = ($_ -split ',')[0]
			$Value = ($_ -split ',')[1]
			Set-RegistryKey -Key 'HKCU\Software\VB and VBA Program Settings\ASAP Utilities\setup' -name $Name -Value $Value -Type String -SID $SID
		}
		#endregion
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$GetInstalled = Get-InstalledApplication -Name "$appVendor"
		...
		} ELSEIF (Test-Path -Path "$envProgramFilesX86\ASAP Utilities\unins000.exe" -PathType Leaf ) {
			Write-Log -Message "Used if-then x86" -Severity 1 -Source $installPhase
			Execute-Process -Path "$envProgramFilesX86\ASAP Utilities\unins000.exe" -Parameters $arguments
		} ELSEIF (Test-Path -Path "$envProgramFiles\ASAP Utilities\unins000.exe" -PathType Leaf) {
			Write-Log -Message "Used if-then" -Severity 1 -Source $installPhase
			Execute-Process -Path "$envProgramFiles\ASAP Utilities\unins000.exe" -Parameters $arguments
		}
