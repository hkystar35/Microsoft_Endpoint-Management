...
Try {
	...
	[string]$appVendor = 'CCH Small Firm Services'
	[string]$appName = 'CCH IntelliForms'
	[string]$appVersion = '18.0.0.0'
	[string]$appMSIProductCode = '{377d05c9-f268-4fdf-8fbb-2a89b12d3aec}'
	...
	[string]$appScriptDate = '02/15/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-Process -Path 'ifsetup.exe' -Parameters "/s /v`"/qn`" /v`"/l*v $configToolkitLogDir\$($appName)_$($appVersion).log`""
		Remove-DesktopShortcut -Name "CCH* IntelliForms*" -UseWildCard
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$Uninstall = Execute-MSI -Action Uninstall -Path $appMSIProductCode -PassThru
		$Uninstall2 = Remove-MSIApplications -Name 'CCH IntelliForms' -PassThru

		
		IF($Uninstall.ExitCode -eq 0 -or $Uninstall2.ExitCode -eq 0){
			$Folders = @()
			$envProgramFiles,$envProgramFilesX86 | %{
				$Folders += Get-Item -Path "$_\Wolters Kluwer\IntelliForms" -ErrorAction SilentlyContinue
				$Folders += Get-Item -Path "$_\IntelliForms" -ErrorAction SilentlyContinue
				$Folders += Get-Item -Path "$_\Common Files\CCHSFS" -ErrorAction SilentlyContinue
			}
