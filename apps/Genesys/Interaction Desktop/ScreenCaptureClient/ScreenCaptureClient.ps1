...
Try {
	...
	[string]$appVendor = 'PCTY'
	[string]$appName = 'IC User Applications_ScreenCapture'
	[string]$appVersion = '1.0'
	[string]$appMSIProductCode = $false #''
	[string]$appProcessesString = 'ScreenCaptureClientU' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = 'ScreenCaptureClientU' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '07/25/2018'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$Result = Get-InstalledApplication -Name 'IC User Applications'
		IF ($result.Count -le 1) {
			$GUID = $result.ProductCode
			Write-Log -Message "Here's the result: $GUID "
			Execute-MSI -Action Install -Path "$GUID" -AddParameters "ADDLOCAL=Feature_ScreenRecorderCaptureClient" -SkipMSIAlreadyInstalledCheck
		} ELSEIF ($Result.Count -gt 1) {
			Write-Log -Message "Too many results. Please be more specific.`nThese are the results: $Result"
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		IF ($result.Count -le 1) {
			$GUID = $result.ProductCode
			Write-Log -Message "Here's the result: $GUID "
			Execute-MSI -Action Install -Path "$GUID" -AddParameters "REMOVE=Feature_ScreenRecorderCaptureClient" -SkipMSIAlreadyInstalledCheck
		} ELSEIF ($Result.Count -gt 1) {
			Write-Log -Message "Too many results. Please be more specific.`nThese are the results: $Result"
		}
