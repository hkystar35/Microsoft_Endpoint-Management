...
Try {
	...
	[string]$appVendor = 'Interactive Intelligence Inc.'
	[string]$appName = 'IC User Applications (64-bit) 2017 R1'
	[string]$appVersion = '17.1.8.24'
	[string]$appMSIProductCode = '{38729C79-C066-4BFB-A6BE-8BD59B82227B}'
	[string]$appProcessesString = 'InteractionClient,InteractionDesktop,InteractionFax,InteractionVoicemail,InteractionVoicemailLauncherU,SIPSoftPhone,ScreenCaptureClientU' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = $false #'' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '08/21/2018'
	...
	If ($deploymentType -ine 'Uninstall') {
        ...
        #{478807C2-7DD9-4F09-8425-503993416FB1}  Interactive Update Client 2015 R2      Interactive Intelligence, Inc.  15.2.6.30
        #{9D07ED93-2963-4A32-BA68-61B9308855FA}  IC User Applications (64-bit) 2015 R2  Interactive Intelligence, Inc.  15.2.6.30
		...
		# Removing IC apps first for clean install
		$Baseversion = '17.1.0.330'
		$GetInstalled = Get-InstalledApplication -Name "IC User Applications"
		
		IF ([System.Version]$GetInstalled.DisplayVersion -lt [System.Version]$Baseversion) {
			Write-Log -Message "$appName prior to $Baseversion detected, performing full removal." -Severity 1 -Source $installPhase
			Remove-MSIApplications -Name "IC User Applications"
			Remove-MSIApplications -Name "Interactive Update Client"
			Write-Log -Message "Removed old versions. Installing latest." -Severity 1 -Source $installPhase
			Execute-MSI -Action Install -Path 'ICUserApps_64bit_2017_R1.msi' -Transform 'Normal.mst' -Patch 'ICUserApps_64bit_2017_R1_Patch8.msp'
		} ELSEIF (([System.Version]$GetInstalled.DisplayVersion -ge [System.Version]$Baseversion) -and ([System.Version]$GetInstalled.DisplayVersion -lt [System.Version]$appVersion)) {
			Write-Log -Message "Found $appName version between $Baseversion and $appVersion. Applying patch only." -Severity 1 -Source $installPhase
			Execute-MSP -Path 'ICUserApps_64bit_2017_R1_Patch8.msp'
		}
		...
		Execute-MSI -Action Install -Path 'Adapt.InteractionDesktop.AgentAutomatedStatusChanger.Setup.msi'
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Execute-MSI -Action Uninstall -Path '{38729C79-C066-4BFB-A6BE-8BD59B82227B}'
        Remove-MSIApplications -Name "IC User Applications"
        Remove-MSIApplications -Name "Interactive Update Client"
        Remove-MSIApplications -Name "Adapt - Interaction Desktop - Agent Automated Status Changer"
		Execute-MSI -Action Uninstall -Path '{41EAC72A-9980-41FE-A793-420C9C057F16}'
