...
(
	...
	[Parameter(Mandatory = $true)][ValidateNotNullOrEmpty()][ValidatePattern('^[0-9a-zA-Z]{3}[0-9a-fA-F]{5}$')][string]$OSDPackageID,
...
Try {
	...
	[string]$appName = 'Windows 10 Upgrade'
	[string]$appVersion = ''
	[string]$appMSIProductCode = ''
	[string]$appProcessesString = '' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = '' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '03/21/2018'
	...
	# Do not move this function to the extensions ps1, it needs to stay here
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$dom = $LoggedOnUser.DomainName
		$usr = $LoggedOnUser.UserName
		$UserName = [adsi]"WinNT://$dom/$usr,user" | Select-Object fullname, name
		$SplitName = $UserName.FullName -split ' '
		...
		#Workflow, too
		#Email task in Epic
		#COntent Validation tool for DPs
		
		# Backup data
		#   OneDrive
		# Completion RegKey to check for in TS and Scheduled Task script
		# Calendar invite
		
		
		[datetime]$DeadlineDate = '05/21/2018 17:00:00'
		#$OSDPackageID = 'PAY004E7'
		
		# 
		
		Show-InstallationWelcome -PersistPrompt -DeferDeadline $DeadlineDate
		$Prompt1 = Show-InstallationPrompt -Message "Hello, $FullName!`n`nWe are upgrading your computer to Windows 10.`n`nDo you want to do this now?`n(It will take approximately *1 hour*.)" -ButtonLeftText 'Yes!' -ButtonRightText "Let's reschedule"
		if ($Prompt1 -eq 'Yes!') {
			Show-InstallationPrompt -Message "Thanks, $GivenName! Click OK to get started and grab some lunch, we should be done in about an hour.`n`nP.S - If you have a question, here's the number to the Help Desk:`n888-PAYLOCITY (888-729-5624)" -ButtonMiddleText 'OK'
			
			#Call TS
			Invoke-OSDInstall -OSDPackageID $OSDPackageID
			
			
		}
		if ($Prompt1 -eq "Let's reschedule") {
			[datetime]$ChooseDate = Choose-Date
			$Time = [System.Timespan]::Parse("9:00")
			$DeferDate = $ChooseDate.Add($Time)
			Set-RegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\Paylocity\InPlaceUpgrade' -Name 'DeferDate' -Value $DeferDate
			
		}
		# emails to tag ticekts with unique identifier
		# can defer after selection up to Deadline Date
		# 
