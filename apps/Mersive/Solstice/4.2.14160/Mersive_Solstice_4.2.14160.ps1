...
Try {
    ...
    [string]$appName = 'Solstice Client'
    [string]$appVersion = '4.2.14160'
    [string]$appMSIProductCode = '{337eaa33-1c03-4e45-b77d-0ebb3d9b9c54}'
    ...
    [string]$appScriptDate = '08/06/2019'
    ...
    If ($deploymentType -ine 'Uninstall') {
        ...
        $SDSServer = 'int-mersive-01.paylocity.com'
        Execute-MSI -Action Install -Path 'SolsticeClient-4.2.14160.msi' -AddParameters "STARTONLOGIN=0 HIDEUIWHENSHARING=1 SDSADDRESS=$($SDSServer)" -Transform 'Solstice.mst'
        ...
        $Protocols = "TCP", "UDP"
		$SolsticeClient = Get-InstalledApplication -Name 'Solstice Client' | Select -First 1
		$TargetFile = Join-Path -Path $($SolsticeClient.InstallLocation) -ChildPath 'solsticeclient.exe'
		$FirewallRuleDescription = "Firewall exclusion for Mersive Solstice Client, a screen mirroring service."
		$FirewallDisplayName = "Solstice Client"
        ...
        IF ($SolsticeClient) {
            ...
            IF (Test-Path -Path $TargetFile -PathType Leaf) {
                Foreach ($Protocol in $Protocols) {
                    $NewFirewallRule = New-NetFirewallRule -DisplayName "$FirewallDisplayName" -Description "$FirewallRuleDescription" -Direction Inbound -Program "$TargetFile" -Protocol $Protocol -Action Allow -EdgeTraversalPolicy DeferToUser -Enabled True -Profile Private,Domain
                    IF ($NewFirewallRule) {
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        Remove-MSIApplications -Name 'Solstice Client'
