...
Try {
	...
	[string]$appVersion = '2.8.9'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '06/30/2017'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'solsticeclient_v2' -AllowDefer -DeferTimes 3 -CheckDiskSpace -PersistPrompt
		...
		Execute-Process -Path 'SolsticeClientSetup_V2-2.8.9_SCCM.exe' -Arguments "/s /f1`"$dirFiles\setup_install.iss`""
        ...
        # Create FireWall rule to allow, and remove previous rules
            $DisplayName = "solsticeclient_v2"
            $Description = $DisplayName
            $TargetFile = "$envProgramFilesx86\mersive technologies inc\solstice\client_v2\solsticeclient_v2.exe"
            $Protocols = "TCP","UDP"
        ...
        #Shortcut
        Remove-File -Path "$envCommonDesktop\Solstice Client_V2.lnk"
        $SDS = "10.129.10.116"
        New-Shortcut -Path "$envCommonDesktop\Solstice Client_V2.lnk" -TargetPath "`"$TargetFile`"" -Arguments "-sdsaddress $SDS" -IconLocation 'C:\Program Files (x86)\Mersive Technologies Inc\Solstice\Client_V2\SolsticeClient_V2.exe' -Description "Solstice Client_V2"
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'solsticeclient_v2' -CloseAppsCountdown 300
		...
		Execute-Process -Path 'SolsticeClientSetup_V2-2.8.9_SCCM.exe' -Parameters "/x /s /f1`"$dirFiles\setup_uninstall.iss`""
        #Execute-Process -Path "$envProgramFilesX86\InstallShield Installation Information\{c6e22d3f-ded6-4fd9-ac1c-5275cc2c4710}\setup.exe" -Parameters '-runfromtemp -l0x0409  -removeonly'
        #Execute-MSI -Path '{c6e22d3f-ded6-4fd9-ac1c-5275cc2c4710}' -Action Uninstall
