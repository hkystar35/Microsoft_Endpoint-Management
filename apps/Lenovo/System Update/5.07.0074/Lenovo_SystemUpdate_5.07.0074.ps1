...
Try {
	...
	[string]$appName = 'System Update'
	[string]$appVersion = '5.07.0074'
	[string]$appMSIProductCode = $false #''
	[string]$appProcessesString = 'SUService,Tvsukernel' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-process -path 'systemupdate5.07.0074.exe' -parameters '/verysilent'
		Execute-Process -Path 'systemupdate5.07.0074.exe' -Parameters "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /SP- /LOG=`"$configToolkitLogDir\$($appName)_$($appVersion).log`" /NOCANCEL /RESTARTEXITCODE=3010 /SAVEINF=`"$configToolkitLogDir\$($appName)_$($appVersion).inf`" /NOICONS"
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Execute-Process -Path "$envProgramFilesX86\Lenovo\System Update\unins000.exe" -Parameters "/SILENT"
