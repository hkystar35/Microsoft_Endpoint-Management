...
Try {
	...
	[string]$appScriptDate = '01/03/2018'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-Process -Path 'systemupdate5.07.0065.exe' -Parameters "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /SP- /LOG=`"$configToolkitLogDir\$($appName)_$($appVersion).log`" /NOCANCEL /RESTARTEXITCODE=3010 /SAVEINF=`"$configToolkitLogDir\$($appName)_$($appVersion).inf`" /NOICONS"
		# /LOG=`"$configToolkitLogDir\$appName-$Installversion.log`"
