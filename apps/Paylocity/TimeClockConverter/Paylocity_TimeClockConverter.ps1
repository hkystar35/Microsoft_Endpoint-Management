...
Try {
	...
	[string]$appName = 'TimeClockConverter'
	[string]$appVersion = ''
	[string]$appMSIProductCode = $false #''
	[string]$appProcessesString = 'PaylocityTimeSheetConverter' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		# "Install" Software
		$InstalledPath = (New-Item -Path "$envProgramFilesX86\Paylocity\TimeClockConverter" -ItemType Directory -Force -Confirm:$false -ErrorAction Stop).FullName
		Copy-File -Path $dirFiles\Files -Destination $InstalledPath -Recurse
		
		# Run updater
		IF($CurrentLoggedOnUserSession.IsRdpSession -eq $true){
			Write-Log -Message "User $($CurrentLoggedOnUserSession.DomainName)\$($CurrentLoggedOnUserSession.username) logged on in RDP Session. Exiting."
			Exit-Script -ExitCode 691000
		}
		$ProfilePath = Get-UserProfiles | ?{$_.SID -eq $CurrentLoggedOnUserSession.SID} | select -ExpandProperty ProfilePath
		$XMLFile
		Copy-File -Path $ProfilePath
