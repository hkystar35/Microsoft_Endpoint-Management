...
Try {
	...
	[string]$appName = 'PDFSplitter'
	[string]$appVersion = '1.2.0'
	[string]$appMSIProductCode = '{B8068F6D-FD1E-46F3-B376-B4BB1C6E6085}'
	[string]$appProcessesString = 'PDFSplitter' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = $false #'' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '06/14/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-MSI -Action Install -Path 'PDFSplitter.msi'

		## Create Start Menu folder Shortcut
		$File = Get-ChildItem -Path "$envProgramFilesX86\Paylocity\*" -Filter pdfsplitter.exe -Recurse -ErrorAction SilentlyContinue | select -First 1 -ErrorAction SilentlyContinue

		New-Shortcut -Path "$envProgramData\Microsoft\Windows\Start Menu\Programs\$($appVendor)\$($File.BaseName).lnk" -TargetPath "$($File.FullName)" -IconLocation "$($File.FullName)" 
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		# Get exe file first
		$FileName = 'PDFSplitter'
		$Files = Get-ChildItem -Path "$envProgramData\Microsoft\Windows\Start Menu\Programs\*" -Filter $($FileName).lnk -Recurse -ErrorAction SilentlyContinue
		...
		$Files | %{
			IF($_.BaseName -eq $FileName -and (Test-Path -Path $_.FullName -PathType Leaf -ErrorAction SilentlyContinue)){
				Remove-File -Path $_.FullName
				Write-Log -Message "File: $($_.FullName)"
			}
		}
