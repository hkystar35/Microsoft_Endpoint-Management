...
Try {
	...
	[string]$appName = 'Security Awareness Screen Saver'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$SmartScreen_StartingStatus = Get-SmartScreenSettingsStatus
		IF ($SmartScreen_StartingStatus -ne 'off') {
			Set-SmartScreenSettingsStatus -State Off
		}

				$exeFile = Get-Item "$dirFiles\SecSCRN.exe" -ErrorAction Stop
				$exeDestination = Copy-Item -Path $exeFile.FullName -Destination "$envWinDir\SysWOW64" -Force -PassThru

				$RegularInstall = Execute-Process -Path $exeDestination.FullName -Parameters '/q' -IgnoreExitCodes 123 -PassThru

				IF($RegularInstall.ExitCode -ne 0){
					Write-Log -Message "Regular install failed. Trying copy operation and setting registry keys."
					Remove-File -Path $exeDestination.Fullname
					$scrFile = Get-Item "$dirFiles\SecSCRN.scr" -ErrorAction Stop
					$scrDestination = Copy-Item -Path $scrFile.FullName -Destination "$envWinDir\SysWOW64" -Force -PassThru
					[scriptblock]$HKCURegistrySettings = {
						Set-RegistryKey -Key "HKCU\Control Panel\Desktop" -Name 'SCRNSAVE.EXE' -Value "$($scrDestination.FullName)" -SID $UserProfile.SID
					}
					Invoke-HKCURegistrySettingsForAllUsers -RegistrySettings $HKCURegistrySettings
				}
		
		#Re-enbale File Security  
		Remove-Item env:\SEE_MASK_NOZONECHECKS
		IF($SmartScreen_StartingStatus){
			Set-SmartScreenSettingsStatus -State $($SmartScreen_StartingStatus)
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		[scriptblock]$HKCURegistrySettings = {
					Remove-RegistryKey -Key "HKCU\Control Panel\Desktop" -Name 'SCRNSAVE.EXE' -SID $UserProfile.SID
					Remove-RegistryKey -Key "HKCU\Control Panel\Desktop" -Name 'ScreenSaveActive' -SID $UserProfile.SID
					Remove-RegistryKey -Key "HKCU\Control Panel\Desktop" -Name 'ScreenSaverIsSecure' -SID $UserProfile.SID
					Remove-RegistryKey -Key "HKCU\Software\Policies\Microsoft\Windows\Control Panel\Desktop" -Name 'SCRNSAVE.EXE' -SID $UserProfile.SID
				}
		Invoke-HKCURegistrySettingsForAllUsers -RegistrySettings $HKCURegistrySettings
    
		$File = 'SecSCRN.exe'
		$Uninstallers = 'SysWOW64','system32' | %{Get-ChildItem -Path $envWinDir\$_ -Filter $($File) -ErrorAction SilentlyContinue}
		$Uninstallers | %{
			Execute-Process -Path $_.FullName -Parameters '/us' -PassThru
			Remove-File -Path $_.FullName
		}
		$ScreenSaver = 'SecSCRN.scr'
		$ScreenSavers = 'SysWOW64','system32' | %{Get-ChildItem -Path $envWinDir\$_ -Filter $($ScreenSaver) -ErrorAction SilentlyContinue}
		$ScreenSavers | %{
			Remove-File -Path $_.FullName
		}
