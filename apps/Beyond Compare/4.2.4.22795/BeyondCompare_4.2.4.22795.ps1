...
Try {
	...
	[string]$appVendor = 'Scooter Software, Inc.'
	[string]$appName = 'Beyond Compare 4'
	[string]$appVersion = '4.2.4.22795'
	[string]$appMSIProductCode = '{382FD58E-226F-418B-8F34-DA8EE89D9550}'
	[string]$appProcessesString = 'bcompare' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = '' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '06/07/2018'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-MSI -Action Install -Path 'BCompare-4.2.4.22795_x64.msi'
		...
		# Copy license file
		[string]$InstallSource = IF($Path = Get-ChildItem -Path ${env:ProgramFiles(x86)} -Filter "$($appProcessesString).exe" -Recurse){$Path.DirectoryName}ELSEIF($Path = Get-ChildItem -Path $env:ProgramFiles -Filter "$($appProcessesString).exe" -Recurse){$Path.DirectoryName}
		$FileName = 'BC4Key.txt'
		# Register License for All Users
		if ($InstallSource) {
			Copy-File -Path "$dirFiles\$FileName" -Destination "$InstallSource"
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		# Ensure License Key is deleted
		$FileName = 'BC4Key.txt'
		[string]$FilePath = IF($Path = Get-ChildItem -Path ${env:ProgramFiles(x86)} -Filter "$($FileName)" -Recurse){$Path.FullName}ELSEIF($Path = Get-ChildItem -Path $env:ProgramFiles -Filter "$($FileName)" -Recurse){$Path.FullName}
		Remove-File -Path "$FilePath"
