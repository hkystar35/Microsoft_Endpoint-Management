...
Try {
	...
	[string]$appVendor = 'Absolute Software Inc'
	[string]$appName = 'Computrace'
	[string]$appVersion = '8.0.944'
	[string]$appMSIProductCode = '{6B95EA5F-74EC-4D29-B49A-7521AB6D9A0E}'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$appExitCode = Execute-MSI -Action Install -Path 'Computrace.msi' -PassThru
		if ($appExitCode.ExitCode -eq 0 -or $appExitCode.ExitCode -eq 3010 -or $appExitCode.ExitCode -eq 1641) {
			[string]$installdate = Get-Date -Format yyyyMdd
			Set-RegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Paylocity\AbsoluteSoftwareInc\$appMSIProductCode" -Name 'DisplayName' -Value "$appName"
			Set-RegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Paylocity\AbsoluteSoftwareInc\$appMSIProductCode" -Name 'InstallDate' -Value "$installdate"
			Set-RegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Paylocity\AbsoluteSoftwareInc\$appMSIProductCode" -Name 'DisplayVersion' -Value "$appVersion"
			Set-RegistryKey -Key "HKEY_LOCAL_MACHINE\SOFTWARE\Paylocity\AbsoluteSoftwareInc\$appMSIProductCode" -Name 'Publisher' -Value "$appVendor"
		}
