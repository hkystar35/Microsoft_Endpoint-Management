...
Try {
	...
	[string]$appName = 'BeyondTrust PowerBroker Client for Windows_Uninstall'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		#	Write-Log -Message "Stopped Process $Process." -Severity 1 -Source $installPhase
		#}
		...
		# PB install
		#$RebootWindow = Execute-MSI -Action Install -Path 'PBWC_x64_7.4.0.msi' -AddParameters 'ADDLOCAL=PBWClient,Client_x64,Runtime_x64,SessionMonitor_x64,IEIntegration_x64,CPIntegration,EventMonitor_x64,FileIntegrity_x64 SERVER=BeyondInsight' -PassThru
		#$RebootWindow = Execute-MSI -Action Install -Path 'PBWC_x64_7.3.0.30.msi' -AddParameters 'ADDLOCAL=PBWClient,Client_x64,Runtime_x64,SessionMonitor_x64,IEIntegration_x64,CPIntegration,EventMonitor_x64,FileIntegrity_x64 SERVER=BeyondInsight' -PassThru
		...
		#if ($RebootWindow.ExitCode -eq 3010) {
		#	Write-Log -Message "Exit code 3010, reboot required"
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		#$PBInstalls = Get-InstalledApplication -Name 'PowerBroker' -Wildcard
		
		# Remove cert installer
		...
		# Remove PB	
		$RebootWindow = Remove-MSIApplications -Name "BeyondTrust PowerBroker" -PassThru
		#'{7E873391-084F-4AAC-87CC-375A39A94F91}'
		
		IF($RebootWindow.ExitCode -ne 0 -or $RebootWindow.ExitCode -ne 3010 -or $RebootWindow.ExitCode -ne 1641 -or $RebootWindow.ExitCode -ne 1707 -or $RebootWindow.ExitCode -ne 1618){
			Get-ChildItem -Path $dirFiles -Filter *.msi | %{
				$Finalreboot = Execute-MSI -Action Uninstall -Path $_.FullName -PassThru
				$Finalreboot.ExitCode
			}
		}ELSE{
			$Finalreboot = $RebootWindow
		}
		...
		if ($Finalreboot.ExitCode -eq 3010) {
			Write-Log -Message "Exit code 3010, reboot required"
			Show-InstallationRestartPrompt -Countdownseconds 1200 -CountdownNoHideSeconds 1200
		}
