...
Try {
	...
	[string]$appVersion = '7.4.0.15'
	[string]$appMSIProductCode = '{7E873391-084F-4AAC-87CC-375A39A94F91}'
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		# Remove PB	
		$GetPBInstalls = Get-InstalledApplication -Name "BeyondTrust PowerBroker"
		$GetPBInstalls | ForEach-Object{
			$RebootWindow = Execute-MSI -Action Uninstall -Path $_.ProductCode -PassThru
			IF ($RebootWindow.ExitCode -eq 1612) {
				SWITCH ($_.Is64BitApplication) {
					true {$PBbitness = '64'} 
					false {$PBbitness = '86'}
				}
				$PBVersion = ($_.DisplayVersion).Substring(0, 5)
				$CurrentMSI = "PBWC_x$($PBbitness)_$($PBVersion).msi"
				$TempMSI = "PowerBroker for Windows Client ($($PBbitness)-bit).msi"
				$TempFolder = New-Item -Path "$envHomeDrive\Temp\PB\$($PBVersion)" -ItemType Directory
				IF (Test-Path -Path "$dirFiles\$CurrentMSI") {
					Copy-File -Path $dirFiles\$CurrentMSI -Destination $TempFolder
				}
			}
		}
