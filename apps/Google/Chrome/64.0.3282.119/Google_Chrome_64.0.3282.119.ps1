...
Try {
	...
	[string]$appVendor = 'Google Inc'
	[string]$appName = 'Google Chrome 64.0.3282.119'
	[string]$appVersion = '67.52.32887'
	[string]$appMSIProductCode = '{CDF467F2-C966-3D6E-9D73-3FACF617E6F8}'
	[string]$appProcessesString = 'chrome' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = '' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '01/25/2018'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-Process -path "$envSystem32Directory\reg.exe" -Parameters "import `"$dirFiles\chromesettings.reg`""
