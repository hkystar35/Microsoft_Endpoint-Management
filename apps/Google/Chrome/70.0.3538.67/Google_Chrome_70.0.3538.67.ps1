  ...
  If ($deploymentType -ine 'Uninstall') {
    ...
    foreach($ChromeInstall in $ChromeInstalls){
      IF($ChromeInstall.UninstallString -notlike "*msiexec*"){
		...
		<#
		# Registry key uninstalls
$appkeys = Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* | ?{$_.DisplayName -like '*chrome*'}
If($appkeys){
  Foreach($appkey in $appkeys){

    If($appkey.uninstallString){
      $str = $appkey.uninstallString.Replace('"','')
      # Get Excutable
      $UninstallArguments = $str.substring($str.IndexOf(".exe") +4 )
      # Get Arguements
      $UninstallExecutible = $str.substring(0,$str.IndexOf(".exe") +4 )
      If($appkey.UninstallString -like '*Msiexec.exe*'){
        $command = {msiexec /x $appkey.PSChildName /qn /q /norestart}
        $scriptBlock = [Scriptblock]::Create($command)
        ## Invoke-Command -ScriptBlock $scriptBlock
        ## Start-Sleep -Seconds 20
        #while((Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | ?{$_.PSChildName -eq $appkey.PSChildName})){Start-Sleep -m 300}
        #$RetVal = Start-Process -FilePath "$UninstallExecutible" -ArgumentList "$UninstallArguments /qn /q /norestart" -Wait -Passthru
      }
      Else{
        ## $RetVal = Start-Process -FilePath "$UninstallExecutible" -ArgumentList "$UninstallArguments /silent /norestart" -Wait -Passthru
        #while((Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | ?{$_.PSChildName -eq $appkey.PSChildName})){Start-Sleep -m 300}
        #Start-Sleep -Seconds 25
      }
      Write-host "Uninstalled: $($appkey.DisplayName)" -ForegroundColor Yellow
    }
    Else{Write-host "UninstallString Missing: $($appkey.DisplayName)" -ForegroundColor Green }
  }
}
Else{Write-host "Reg = None"}
