...
Try {
	...
	[string]$appVendor = 'Slack Technologies'
	[string]$appName = 'Slack'
	[string]$appVersion = '3.3.1.0'
	[string]$appMSIProductCode = '{3E4FF3AA-B5A7-4D25-99A8-F11B348565DB}'
	[string]$appProcessesString = 'slack' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = $false #'' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '09/17/2018'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-MSI -Action Install -Path 'slack-standalone-3.3.1.0.msi'
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		# Check for per-user install and remove
			$CurrentUser = Get-LoggedOnUser
			$CurrentUserAppData = "$env:HOMEDRIVE\Users\$($CurrentUser.UserName)\AppData"
			
			$SlackEXEpath = "$CurrentUserAppData\Local\slack\Update.exe"
		
			IF (Test-Path -Path $SlackEXEpath -PathType Leaf) {
				$SlackExit = Execute-Process -Path $SlackEXEpath -Parameters '--uninstall -s' -IgnoreExitCodes -1 -Passthru
				IF ($SlackExit.ExitCode -eq '-1') {
					$DeleteFolders = "$CurrentUserAppData\Local\slack","$CurrentUserAppData\Roaming\Microsoft\Windows\Start Menu\Programs\Slack Technologies"
					FOREACH ($DeleteFolder IN $DeleteFolders) {
						Remove-Folder -Path $DeleteFolder
					}
					Remove-File -Path "$env:HOMEDRIVE\Users\$($CurrentUser.UserName)\Desktop\Slack.lnk"
				}
			}
