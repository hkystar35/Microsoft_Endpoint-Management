...
TRY {
	...
	IF ($deploymentType -ine 'Uninstall') {
		...
		Execute-Process -Path 'CitrixReceiver.exe' -Parameters '/silent /RCU /noreboot /EnableCEIP=False /AutoUpdatecheck=Disabled' -WaitForMsiExec
		#Execute-Process -Path 'CitrixReceiver.exe' -Parameters '/silent /noreboot /EnableCEIP=False /AutoUpdatecheck=Disabled'
		Write-Log -Message "Install log location: $ProfilePath\AppData\Local\Temp\CTXReceiverInstallLogs*" -Severity 1 -Source $installPhase
		#regkey PutShortcutsOnDesktop = false
		...
		$Uninstall = Execute-Process -Path 'CitrixReceiver.exe' -Parameters '/silent /uninstall' -WaitForMsiExec -PassThru
		Write-Log -Message "$Uninstall"
