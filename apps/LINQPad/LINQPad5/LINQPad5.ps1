...
Try {
	...
	[string]$appVendor = 'Joseph Albahari'
	[string]$appName = 'LINQPad 5'
	[string]$appVersion = '5'
	[string]$appMSIProductCode = ''
	[string]$appProcessesString = 'LINQPad,LINQPad.UserQuery' # comma-separated, but all inside sigle quote, like 'chrome,firefox,iexplore'
	[string]$appServicesString = '' # comma-separated, but all inside sigle quote, like 'zoom,ccmexec,spotify'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '06/01/2018'
  ...
  If ($deploymentType -ine 'Uninstall') {
    ...
    $GetInstalled = Get-InstalledApplication -Name "LINQPad 5"
    IF(!$GetInstalled){
			Execute-Process -Path "LINQPad5Setup.exe" -Parameters "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /LOG=`"$($configToolkitLogDir)\$($appVendor)_$($appName)_$($appVersion)_$($installPhase).log`""
    }
  ...
  ElseIf ($deploymentType -ieq 'Uninstall')
  {
    ...
    $GetInstalled = Get-InstalledApplication -Name "LINQPad 5"
    IF($GetInstalled){
      [string]$UninstallFullPath = $GetInstalled.UninstallString -replace '"',''
      Write-Log -Message "Regquery worked" -Severity 1 -Source $installPhase
      Execute-Process -Path "$($UninstallFullPath)" -Parameters "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /LOG=`"$($configToolkitLogDir)\$($appVendor)_$($appName)_$($appVersion)_$($installPhase).log`""
    }ELSEIF(Test-Path -Path "$envProgramFilesX86\LINQPad5\unins000.exe" -PathType Leaf){
      Write-Log -Message "Used if-then" -Severity 1 -Source $installPhase
      Execute-Process -Path "$envProgramFilesX86\LINQPad5\unins000.exe" -Parameters "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /LOG=`"$($configToolkitLogDir)\$($appVendor)_$($appName)_$($appVersion)_$($installPhase).log`""
    }ELSEIF(Test-Path -Path "$envProgramFiles\LINQPad5\unins000.exe" -PathType Leaf){
      Write-Log -Message "Used if-then" -Severity 1 -Source $installPhase
      Execute-Process -Path "$envProgramFiles\LINQPad5\unins000.exe" -Parameters "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /LOG=`"$($configToolkitLogDir)\$($appVendor)_$($appName)_$($appVersion)_$($installPhase).log`""
    }
