...
Try {
  ...
  If ($deploymentType -ine 'Uninstall') {
    ...
    $GetInstalled = Get-InstalledApplication -Name "LINQPad 4"
    IF(!$GetInstalled){
			Execute-Process -Path "LINQPad4Setup.exe" -Parameters "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /LOG=`"$($configToolkitLogDir)\$($appVendor)_$($appName)_$($appVersion)_$($installPhase).log`""
    }
